...
- 다음 단계로 넘기기 전에 `workflow-runtime.md`의 Reviewer verification 최소 검증 기준을 대조한다. 누락된 검증 결과, 미검증 항목, 남은 차이가 있으면 `07-reviewer-verification.md`에 보완한다.
...
- `run-state.json` 갱신은 Root orchestrator가 담당한다. Reviewer는 재검증 필요 여부, Developer에게 돌려보낼 이슈 목록, 남은 차이, 미검증 항목을 보고한다.
